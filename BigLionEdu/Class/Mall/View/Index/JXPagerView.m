...
- (void)initializeViews {
    ...
    self.mainTableView.backgroundColor = [UIColor colorWithRed:249/255.0 green:73/255.0 blue:70/255.0 alpha:1.0];
    ...
    [self.mainTableView setContentInset:UIEdgeInsetsMake(-NavigationHeight(), 0, 0, 0)];
    ...
    [_listContainerView setBackgroundColor:[UIColor whiteColor]];
...
- (void)layoutSubviews {
    ...
    self.mainTableView.frame = CGRectMake(0, NavigationHeight(), CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)-  NavigationHeight());
}
...
- (void)preferredProcessMainTableViewDidScroll:(UIScrollView *)scrollView {
    if (self.currentScrollingListView != nil && self.currentScrollingListView.contentOffset.y > 0) {
        ...
        if (self.needResetTableViewOffset == NO) {
            self.mainTableView.contentOffset = CGPointMake(0, [self.delegate tableHeaderViewHeightInPagerView:self]);
        }
    }
...
- (void)listDidAppear:(NSInteger)index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(currentList:)]) {
        [self.delegate currentList:list];
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(pagerViewDidScroll:index:)]) {
        [self.delegate pagerViewDidScroll:list index:index];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return self.bounds.size.height - NavigationHeight() - [self.delegate heightForPinSectionHeaderInPagerView:self];
}
...
- (void)setNeedResetTableViewOffset:(BOOL)needResetTableViewOffset {
    _needResetTableViewOffset = needResetTableViewOffset;
}
