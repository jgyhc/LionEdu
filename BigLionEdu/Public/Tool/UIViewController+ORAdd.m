...
- (void)or_showOnlySureAlert:(NSString *)title message:(NSString *)msg {
    [self or_showAlert:title message:msg hasCancel:NO okAction:nil cancelAction:nil];
}
...
- (void)or_showAlert:(NSString *)title message:(NSString *)msg hasCancel:(BOOL)hasCancel okAction:(dispatch_block_t)okBlock cancelAction:(dispatch_block_t)cancelBlock {
    [self _or_showAlert:title message:msg actionTitles:@[@"确定"] actionTitles:nil style:UIAlertControllerStyleAlert hasCancel:hasCancel completion:^(NSInteger index) {
        if (okBlock) {
            okBlock();
        }
    } cancel:cancelBlock];
