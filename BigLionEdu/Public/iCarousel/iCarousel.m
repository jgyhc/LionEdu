...
- (CATransform3D)carousel:(__unused iCarousel *)carousel
   itemTransformForOffset:(__unused CGFloat)offset
            baseTransform:(CATransform3D)transform atIdx:(NSInteger)idx { return transform; }
...
//    _contentView.backgroundColor = NT_HEX(#212322);
...
- (CATransform3D)transformForItemViewWithOffset:(CGFloat)offset atIdx:(NSUInteger)idx;
{
    ...
    switch (_type)
    {
        case iCarouselTypeCustom:
        {
            return [_delegate carousel:self itemTransformForOffset:offset baseTransform:transform atIdx:idx];
        }
        case iCarouselTypeLinear:
        {
...
//        containerView.backgroundColor = NT_BLUE_COLOR;
//        view.backgroundColor = NT_YELLOW_COLOR;
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //    [view.superview setFrameOrigin:NSMakePoint(self.bounds.size.width/2.0 + _contentOffset.width,28];
    ...
    //calculate transform
    CATransform3D transform = [self transformForItemViewWithOffset:offset atIdx:index];
