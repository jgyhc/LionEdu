...
//    _tagMargin = 10;
    ...
    _horizontalTagMargin = 30;
    _verticalTagMargin = 5;
    _tagCornerRadius = 5;
    _lineSpacing = 15;
    _interitemSpacing = 25;
    _borderWidth = 0;
...
- (CGFloat)tagListH
{
    ...
    return CGRectGetMaxY([self.tagButtons.lastObject frame]) + _interitemSpacing;
}
...
- (void)addTag:(NSString *)tagStr
{
    ...
    if (_tagClass == nil) {
        tagButton.margin = _horizontalTagMargin;
    }
...
{
    if (_clickTagBlock) {
        _clickTagBlock(button, button.currentTitle);
    }
}
...
- (void)setupTagButtonRegularFrame:(UIButton *)tagButton
{
    ...
    NSInteger margin = (self.bounds.size.width - _tagListCols * btnW - 2 * _interitemSpacing) / (_tagListCols - 1);
    CGFloat btnX = _interitemSpacing + col * (btnW + margin);;
    CGFloat btnY = _lineSpacing + row * (btnH + margin);
...
{
    // 等于上一个按钮的最大X + 间距
    CGFloat btnX = CGRectGetMaxX(preButton.frame) + _interitemSpacing;
    
    // 等于上一个按钮的Y值,如果没有就是标签间距
    CGFloat btnY = preButton? preButton.frame.origin.y : _lineSpacing;
    ...
    CGFloat btnW = extreMargin?titleW + 2 * _horizontalTagMargin : tagButton.bounds.size.width ;
    if (_tagDeleteimage && extreMargin == YES) {
        btnW += imageViewWH;
        btnW += _horizontalTagMargin;
    }
    
    // 获取按钮高度
    CGFloat btnH = extreMargin? titleH + 2 * _verticalTagMargin:tagButton.bounds.size.height;
    if (_tagDeleteimage && extreMargin == YES) {
        CGFloat height = imageViewWH > titleH ? imageViewWH : titleH;
        btnH = height + 2 * _verticalTagMargin;
    }
    ...
    if (rightWidth < btnW) {
        // 不够显示，显示到下一行
        btnX = _interitemSpacing;
        btnY = CGRectGetMaxY(preButton.frame) + _lineSpacing;
    }
