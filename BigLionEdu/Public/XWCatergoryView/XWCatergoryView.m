...
- (void)xwp_interpolationForBottomLineWithRatio:(CGFloat)ratio{
    ...
    CGFloat itemWidth = [self xwp_interpolationFromValue:_fromModel.cellFrame.size.width toValue:_toModel.cellFrame.size.width ratio:ratio - (int)ratio];
    CGFloat y = CGRectGetMaxY(_fromModel.cellFrame) + _bottomLineSpacingFromTitleBottom;
    CGFloat width = _bottomLineLength?:itemWidth;
    CGFloat height = _bottomLineWidth;
    if (_bottomLineLength) {
        x += itemWidth/2.f-_bottomLineLength/2.f;
    }
    _bottomLine.cornerRadius = _bottomLineRadius;
    _bottomLine.frame = CGRectMake(x, y - height, width, height);
    if (self.delegate && [self.delegate respondsToSelector:@selector(catergoryView:scrollToItem:)]) {
        [self.delegate catergoryView:self scrollToItem:(int)ratio];
    }
}
...
- (void)xwp_interpolationForBackEllipseWithRatio:(CGFloat)ratio{
    ...
    CGFloat width = CGSizeEqualToSize(CGSizeZero, _fromModel.backEllipseSize)?_fromModel.backEllipseFrame.size.width:[self xwp_interpolationFromValue:_fromModel.backEllipseFrame.size.width toValue:_toModel.backEllipseFrame.size.width ratio:ratio - (int)ratio];
...
- (void)xwp_updateBottomLineWhenClickedWithIndexPath:(NSIndexPath *)indexPath{
    ...
    [UIView animateWithDuration:_clickedAnimationDuration animations:^{
        CGFloat width = _bottomLineLength?:model.cellSize.width;
        _bottomLine.frame = CGRectMake(_bottomLineLength?model.cellFrame.origin.x+(model.cellSize.width/2.f-_bottomLineLength/2.f):model.cellFrame.origin.x, _bottomLine.y, width, _bottomLine.height);
    }];
}
...
//    av.backgroundColor = [UIColor nt_colorWithHexString:@"#F5F7FA"];
//    [cell sendSubviewToBack:av];
//    av.cornerRadius = 12.5;
//    [cell.contentView addSubview:av];
//    [av mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.center.equalTo(cell.titleLabel);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(catergoryView:canSelectItem:)]) {
        BOOL canScroll = [self.delegate catergoryView:self canSelectItem:indexPath.item];
        if (!canScroll) {
            return;
        }
    }
...
- (void)kp_scrollToWithIndex:(NSInteger)index {
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:index inSection:0];
    self.lastIndexPath = nil;
    [self collectionView:self.mainView didSelectItemAtIndexPath:indexPath];
}
