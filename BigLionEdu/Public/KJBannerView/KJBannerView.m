...
- (void)setImageDatas:(NSArray *)imageDatas{
    ...
    _temps = self.infiniteLoop ? _imageDatas.count * 10 : _imageDatas.count;
...
- (void)scrollToIndex:(NSInteger)index{
    if(index >= _temps){ //滑到最后则调到中间
        if(self.infiniteLoop){
            ...
            if ([self.delegate respondsToSelector:@selector(kj_BannerView:showIndex:)]) {
                long itemIndex = (int)(index) % self.imageDatas.count;
                [self.delegate kj_BannerView:self showIndex:itemIndex];
            }
        }
        return;
    }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset{
    ...
    if ([self.delegate respondsToSelector:@selector(kj_BannerView:showIndex:)]) {
        long itemIndex = (int)(currentIndex + _dragDirection) % self.imageDatas.count;
        [self.delegate kj_BannerView:self showIndex:itemIndex];
    }
}
